...
class OpenalexDBRaw(db.Model):
    ...
    def __repr__(self):
        return "<{} ({}) '{}' {}>".format(self.__class__.__name__, self.issn_l,
                                          self.display_name, self.publisher)
...
class JournalMetadata(db.Model):
    ...
    def set_is_gold_journal_in_most_recent_year(self, journal_raw):
        ...
        if journal_raw.is_oa is not None or journal_raw.is_in_doaj is not None:
            self.is_gold_journal_in_most_recent_year = any(
                [journal_raw.is_oa, journal_raw.is_in_doaj])
    ...
    def set_is_currently_publishing(self, journal_raw):
        ...
        if match:
            if match.date_last_doi:
                date_last_doi_as_date = datetime.datetime.strptime(
                    match.date_last_doi, "%Y-%m-%d")
            ...
            if journal_raw.counts_by_year:
                ...
                for row in dois:
                    if row['year'] == this_year_ish() and row[
                        'works_count'] > 0:
                        self.is_currently_publishing = True
    ...
    def get_subscription_price(self, currency="USD",
...
class JournalConcepts(object):
    def __init__(self, journal_raw):
        ...
        self.x_concepts = safer_json_decode(
            journal_raw.x_concepts)
        self.data = None
        self.set_data()
...
def set_data(self):
    if self.x_concepts:
        level_zero_one_concepts = list(
            filter(lambda x: x['level'] in (0, 1), self.x_concepts))
        for concept in level_zero_one_concepts:
            ...
            concept['wikidata_id'] = concept['wikidata_url'].split('/')[
                -1] if concept['wikidata_url'] else None
        res = [sorted(w.items(), key=lambda pair: self.keys_map()[pair[0]])
               for w in level_zero_one_concepts]
...
def recompute_journal_metadata():
    ...
    with get_db_cursor() as cursor:
        ...
        cursor.execute(
            "create table openalex_computed_bak_yesterday as (select * from openalex_computed);")
        cursor.execute("drop table openalex_concepts_bak_yesterday;")
        cursor.execute(
            "create table openalex_concepts_bak_yesterday as (select * from openalex_concepts);")
    ...
    print(
        "done writing to db, took {} seconds total".format(elapsed(start_time)))


print("loading all journal metadata...", end=' ')
start_time = time()
all_journal_metadata_list = JournalMetadata.query.all()
[db.session.expunge(my_journal_metadata) for my_journal_metadata in
 all_journal_metadata_list]
all_journal_metadata = dict(list(
    zip([journal_object.issn_l for journal_object in all_journal_metadata_list],
        all_journal_metadata_list)))
...
class MissingJournalMetadata(object):
    def __init__(self, issn_l):
        ...
        if issn_l not in oa_issns:
            print(
                "MissingJournalMetadata: missing {} from openalex: https://api.openalex.org/venues/issn:{}".format(
                    issn_l, issn_l))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--recompute", help="Update journal metadata",
