...
scenario_id = 'CUc7kqN8' # Name: "Sage-HopeCollege"
...
def test_updating_scenario_name(fetch_jwt):
    ...
    # jwt = fetch_jwt4testing(os.environ["UNSUB_USER2_PWD"], os.environ["UNSUB_USER2_EMAIL"])
    ...
    changed = requests.get(
        url_base + f"/package/{my_saved_scenario.package_id}",
        headers={"Authorization": "Bearer " + jwt}
    )
    scenario = [x for x in changed.json()['scenarios'] if x['id'] == scenario_id][0]
    assert scenario['name'] == new_scenario_name
