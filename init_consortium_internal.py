...
def create_consortium_id():
	return 'institution-{}'.format(shortuuid.uuid()[0:12])

def make_consortium(publisher, institution_id):
	click.echo("    Adding new institution to jump_institution table")
	with get_db_cursor() as cursor:
		cmd = """insert into jump_institution values (%(inst)s, 'consortiumregulartest', 'Regular Consortium for Testing', sysdate, true, null, false);"""
...
def fetch_package_ids():
	with get_db_cursor() as cursor:
		cmd = "select package_id from jump_account_package where package_id ilike 'package-testing%'"
		cursor.execute(cmd)
		rows = cursor.fetchall()
	
	pkgids = [r[0] for r in rows]
	return pkgids
...
def assoc_feeder_pkgs(publisher, consortium_package_id, feed_ids):
	values = [(f"regularconsortiumtesting_{publisher}", consortium_package_id, w,) for w in feed_ids]
...
# heroku local:run python init_consortium_internal.py --publisher=elsevier
def create_consortium(publisher, pkgid, pkgidprefix):
	"""Create a consortium for internal testing purposes"""
	publisher = publisher.lower()

	consortium_institution_id = create_consortium_id()
	click.echo(f"Using consortium institution ID: '{consortium_institution_id}'")

	consortium_package_id = make_consortium(publisher, consortium_institution_id)
	...
	click.echo("Fetching package IDs")
	package_ids = pkgid
	if not pkgid:
	...
	assoc_feeder_pkgs(publisher, consortium_package_id, feedids)
